...
import (
	...
	"github.com/cidas/cidas/accounts/keystore"
...
// PublicCidasAPI provides an API to access Cidas related information.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicCidasAPI struct {
	b Backend
}

// NewPublicCidasAPI creates a new Etheruem protocol API.
func NewPublicCidasAPI(b Backend) *PublicCidasAPI {
	return &PublicCidasAPI{b}
}

// GasPrice returns a suggestion for a gas price.
func (s *PublicCidasAPI) GasPrice(ctx context.Context) (*big.Int, error) {
	return s.b.SuggestPrice(ctx)
}

// ProtocolVersion returns the current Cidas protocol version this node supports
func (s *PublicCidasAPI) ProtocolVersion() hexutil.Uint {
...
func (s *PublicCidasAPI) Syncing() (interface{}, error) {
...
//   keccak256("\x19Cidas Signed Message:\n"${message length}${message}).
//
// This gives context to the signed message and prevents signing of transactions.
func signHash(data []byte) []byte {
	msg := fmt.Sprintf("\x19Cidas Signed Message:\n%d%s", len(data), data)
	return crypto.Keccak256([]byte(msg))
}

// Sign calculates an Cidas ECDSA signature for:
// keccack256("\x19Cidas Signed Message:\n" + len(message) + message))
//
...
//
// https://github.com/cidas/cidas/wiki/Management-APIs#personal_sign
...
// the address of:
// hash = keccak256("\x19Cidas Signed Message:\n"${message length}${message})
// addr = ecrecover(hash, signature)
//
// Note, the signature must conform to the secp256k1 curve R, S and V values, where
// the V value must be be 27 or 28 for legacy reasons.
//
// https://github.com/cidas/cidas/wiki/Management-APIs#personal_ecRecover
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	if len(sig) != 65 {
		return common.Address{}, fmt.Errorf("signature must be 65 bytes long")
	}
	if sig[64] != 27 && sig[64] != 28 {
		return common.Address{}, fmt.Errorf("invalid Cidas signature (V is not 27 or 28)")
	}
...
// PublicBlockChainAPI provides an API to access the Cidas blockchain.
// It offers only methods that operate on public data that is freely available to anyone.
...
//
// https://github.com/cidas/wiki/wiki/JSON-RPC#eth_sign
...
// Version returns the current cidas protocol version.
