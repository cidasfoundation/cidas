...
// Package utils contains internal helper functions for cidas commands.
package utils
...
// for Cids and returns half of the allowance to assign to the database.
...
func SetNodeConfig(ctx *cli.Context, cfg *node.Config) {
	...
	case ctx.GlobalBool(DevModeFlag.Name):
		cfg.DataDir = filepath.Join(os.TempDir(), "cidas_dev_mode")
	case ctx.GlobalBool(TestnetFlag.Name):
...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *eth.Config) {
	...
	// Cidas needs to know maxPeers to calculate the light server peer ratio.
	// TODO(fjl): ensure Cidas can get MaxPeers from node.
	cfg.MaxPeers = ctx.GlobalInt(MaxPeersFlag.Name)
...
// RegisterEthService adds an Cidas client to the stack.
func RegisterEthService(stack *node.Node, cfg *eth.Config) {
	...
	if err != nil {
		Fatalf("Failed to register the Cidas service: %v", err)
	}
}
...
// RegisterEthStatsService configures the Cidas Stats daemon and adds it to
// th egiven node.
func RegisterEthStatsService(stack *node.Node, url string) {
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		// Retrieve both eth and les services
		var ethServ *eth.Cidas
		ctx.Service(&ethServ)
		...
		Fatalf("Failed to register the Cidas Stats service: %v", err)
	}
}
...
// new format.
//
// e.g. cids account new --keystore /tmp/mykeystore --lightkdf
//
// is equivalent after calling this method with:
//
// cids --keystore /tmp/mykeystore --lightkdf account new
//
