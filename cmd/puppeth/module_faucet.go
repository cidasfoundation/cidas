...
RUN \
  apk add --update git go make gcc musl-dev ca-certificates linux-headers                             && \
	mkdir -p $GOPATH/src/github.com/cidas                                                            && \
	(cd $GOPATH/src/github.com/cidas && git clone --depth=1 https://github.com/cidas/cidas) && \
  go build -v github.com/cidas/cidas/cmd/faucet                                              && \
