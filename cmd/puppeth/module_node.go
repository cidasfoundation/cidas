...
// nodeDockerfile is the Dockerfile required to run an Cidas node.
var nodeDockerfile = `
FROM cidas/client-go:alpine-develop
...
{{end}}
RUN \
  echo '/cids init /genesis.json' > cids.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.cidas/keystore/ && cp /signer.json /root/.cidas/keystore/' >> cids.sh && \{{end}}
	echo $'/cids --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --maxpeers {{.Peers}} {{.LightFlag}} --ethstats \'{{.Ethstats}}\' {{if .BootV4}}--bootnodesv4 {{.BootV4}}{{end}} {{if .BootV5}}--bootnodesv5 {{.BootV5}}{{end}} {{if .Etherbase}}--etherbase {{.Etherbase}} --mine{{end}}{{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --targetgaslimit {{.GasTarget}} --gasprice {{.GasPrice}}' >> cids.sh

ENTRYPOINT ["/bin/sh", "cids.sh"]
`

// nodeComposefile is the docker-compose.yml file required to deploy and maintain
// an Cidas node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
...
// deployNode deploys a new Cidas node container to a remote machine via SSH,
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	var out []byte
	if out, err = client.Run(fmt.Sprintf("docker exec %s_%s_1 /cids --exec admin.nodeInfo.id attach", network, kind)); err != nil {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.cidas"],
