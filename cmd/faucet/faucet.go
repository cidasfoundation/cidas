...
var (
	...
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the Cidas protocol")
	...
	logFlag = flag.Int("loglevel", 3, "Log level to use for Cidas and the faucet")
)
...
type request struct {
	...
	Account  common.Address     `json:"account"`  // Cidas address being funded
	Time     time.Time          `json:"time"`     // Timestamp when te request was accepted
	Tx       *types.Transaction `json:"tx"`       // Transaction funding the account
}

// faucet represents a crypto faucet backed by an Cidas light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // Cidas protocol stack
	client *ethclient.Client   // Client connection to the Cidas chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the Cidas light client protocol
...
// close terminates the Cidas connection and tears down the faucet.
...
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	for {
		...
		// Iterate over all the files and look for Cidas addresses
		...
		if address == (common.Address{}) {
			websocket.JSON.Send(conn, map[string]string{"error": "No Cidas address found to fund"})
			continue
		}
