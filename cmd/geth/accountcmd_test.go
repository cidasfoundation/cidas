...
func TestAccountListEmpty(t *testing.T) {
	cids := runCids(t, "account", "list")
	cids.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	cids := runCids(t, "account", "list", "--datadir", datadir)
	defer cids.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	cids := runCids(t, "account", "new", "--lightkdf")
	defer cids.ExpectExit()
	cids.Expect(`
...
`)
	cids.ExpectRegexp(`Address: \{[0-9a-f]{40}\}\n`)
}
...
func TestWalletImport(t *testing.T) {
	cids := runCids(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer cids.ExpectExit()
	cids.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(cids.Datadir, "keystore"))
	if len(files) != 1 {
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(cids.StderrText(), m) {
...
// https://github.com/cidas/cidas/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	cids.SetTemplateFunc("keypath", func(file string) string {
