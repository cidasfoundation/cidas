...
func (pm *ProtocolManager) Stop() {
	...
	log.Info("Stopping light Cidas protocol")
	...
	log.Info("Light Cidas protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Light Cidas peer registration failed", "err", err)
		return err
	}
	defer func() {
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Light Cidas message handling failed", "err", err)
			return err
		}
	}
}
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	p.Log().Trace("Light Cidas message arrived", "code", msg.Code, "bytes", msg.Size)
