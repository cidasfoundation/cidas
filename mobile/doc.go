...
// Package cids contains the simplified mobile APIs to cidas.
//
// The scope of this package is *not* to allow writing a custom Cidas client
// with pieces plucked from cidas, rather to allow writing native dapps on
...
// from the cidas repository. Please adhere to this scoping to prevent this
// package getting unmaintainable.
//
// Wrapping guidelines:
//
// Every type that is to be exposed should be wrapped into its own plain struct,
// which internally contains a single field: the original cidas version.
