...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Cids.LightScryptN, Cids.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.cidas", new NodeConfig());
			node.start();
			...
			CidasClient ec = node.getCidasClient();
	...
	// signatures too. Regression test for cidas issue #14599.
...
func TestAndroid(t *testing.T) {
	// Skip tests on Windows altocidser
	...
	workspace, err := ioutil.TempDir("", "cids-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/cidas/cidstest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Cids and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.cidas", "github.com/cidas/cidas/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "cids.aar"), "cids.aar", os.ModePerm)

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "cidas", "cidstest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
