...
import (
	...
	"github.com/cidas/cidas/accounts/usbwallet"
...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of cids is "cids". If no
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "Cids", keep that.
	if name == "cids" || name == "cids-testnet" {
		name = "Cids"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "cids" instances.
var isOldCidsResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) resolvePath(path string) string {
	...
	// by cids 1.4 are used if they exist.
	if c.name() == "cids" && isOldCidsResource[path] {
		oldpath := ""
		if c.Name == "cids" {
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	default:
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "cidas-keystore")
		ephemeral = keydir
	}
	if err != nil {
		return nil, "", err
