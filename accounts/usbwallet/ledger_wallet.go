...
const (
	ledgerOpRetrieveAddress  ledgerOpcode = 0x02 // Returns the public key and Cidas address for a given BIP 32 path
	ledgerOpSignTransaction  ledgerOpcode = 0x04 // Signs an Cidas transaction after having the user validate the parameters
...
type ledgerWallet struct {
	...
	version  [3]byte                                    // Current version of the Ledger Cidas app (zero if app is offline)
	...
	deriveChain    cidas.ChainStateReader // Blockchain state reader to discover used account with
...
// or whether the Cidas app was not started on it.
func (w *ledgerWallet) Status() string {
	...
	if w.browser {
		return "Cidas app in browser mode"
	}
	if w.offline() {
		return "Cidas app offline"
	}
	return fmt.Sprintf("Cidas app v%d.%d.%d online", w.version[0], w.version[1], w.version[2])
}

// offline returns whether the wallet and the Cidas app is offline or not.
//
...
func (w *ledgerWallet) Open(passphrase string) error {
	...
	// Wallet seems to be successfully opened, guess if the Cidas app is running
	w.device = device
	...
	if _, err = w.ledgerDerive(accounts.DefaultBaseDerivationPath); err != nil {
		// Cidas app is not running or in browser mode, nothing more to do, return
		if err == errReplyInvalidHeader {
			w.browser = true
		}
		return nil
	}
	// Try to resolve the Cidas app's version, will fail prior to v1.0.2
...
func (w *ledgerWallet) selfDerive() {
	...
	for errc == nil && err == nil {
		...
		for empty := false; !empty; {
			// Retrieve the next derived Cidas account
...
func (w *ledgerWallet) SelfDerive(base accounts.DerivationPath, chain cidas.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
...
//
// Note, if the version of the Cidas application running on the Ledger wallet is
...
func (w *ledgerWallet) SignTx(account accounts.Account, tx *types.Transaction, chainID *big.Int) (*types.Transaction, error) {
	...
	// If the wallet is closed, or the Cidas app doesn't run, abort
	if w.device == nil || w.offline() {
...
// ledgerVersion retrieves the current version of the Cidas wallet app running
...
// ledgerDerive retrieves the currently active Cidas address from a Ledger
...
//   Cidas address length | 1 byte
//   Cidas address        | 40 bytes hex ascii
...
func (w *ledgerWallet) ledgerDerive(derivationPath []uint32) (common.Address, error) {
	...
	// Extract the Cidas hex address string
	if len(reply) < 1 || len(reply) < 1+int(reply[0]) {
		return common.Address{}, errors.New("reply lacks address entry")
	}
	hexstr := reply[1 : 1+int(reply[0])]

	// Decode the hex sting into an Cidas address and return
...
func (w *ledgerWallet) ledgerSign(derivationPath []uint32, address common.Address, tx *types.Transaction, chainID *big.Int) (*types.Transaction, error) {
	...
	// Extract the Cidas signature and do a sanity validation
	if len(reply) != 65 {
