...
import (
	...
	"github.com/cidas/cidas/common/mclock"
...
// Service implements an Cidas netstats reporting daemon that pushes local
// chain statistics up to a monitoring server.
type Service struct {
	stack *node.Node // Temporary workaround, remove when API finalized

	server *p2p.Server        // Peer-to-peer server to retrieve networking infos
	eth    *eth.Cidas      // Full Cidas service if monitoring a full node
	les    *les.LightCidas // Light Cidas service if monitoring a light node
...
func New(url string, ethServ *eth.Cidas, lesServ *les.LightCidas) (*Service, error) {
