...
// NewProtocolManager returns a new cidas sub protocol manager. The Cidas sub protocol manages peers capable
// with the cidas network.
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing Cidas peer", "peer", id)

	// Unregister the peer from the downloader and Cidas peer set
...
func (pm *ProtocolManager) Stop() {
	log.Info("Stopping Cidas protocol")
	...
	log.Info("Cidas protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Cidas peer connected", "name", p.Name())

	// Execute the Cidas handshake
	td, head, genesis := pm.blockchain.Status()
	if err := p.Handshake(pm.networkId, td, head, genesis); err != nil {
		p.Log().Debug("Cidas handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Cidas peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Cidas message handling failed", "err", err)
			return err
		}
	}
}
...
// EthNodeInfo represents a short summary of the Cidas sub-protocol metadata known
// about the host peer.
type EthNodeInfo struct {
	Network    uint64      `json:"network"`    // Cidas network ID (1=Frontier, 2=Morden, Ropsten=3)
