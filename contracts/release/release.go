...
type Config struct {
	Oracle common.Address // Cidas address of the release oracle
...
func NewReleaseService(ctx *node.ServiceContext, config Config) (node.Service, error) {
	// Retrieve the Cidas service dependency to access the blockchain
	var apiBackend ethapi.Backend
	var cidas *eth.Cidas
	if err := ctx.Service(&cidas); err == nil {
		apiBackend = cidas.ApiBackend
	} else {
		var cidas *les.LightCidas
		if err := ctx.Service(&cidas); err == nil {
			apiBackend = cidas.ApiBackend
		} else {
			return nil, err
...
func (r *ReleaseService) checkVersion() {
	...
	if version.Major > r.config.Major ||
		...
		howtofix := fmt.Sprintf("Please check https://github.com/cidas/cidas/releases for new releases")
